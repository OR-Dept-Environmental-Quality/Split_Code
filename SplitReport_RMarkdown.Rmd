...
source("C:/Users/cthomas/Documents/local_R_resources/scripts/Split_Code/Split_Code/ParamGrp_Function.R")
source("C:/Users/cthomas/Documents/local_R_resources/scripts/Split_Code/Split_Code/QC_Issue.R")
logo <- "C:/Users/cthomas/Documents/local_R_resources/scripts/Split_Code/Split_Code/DEQ-logo-color-notransp-horiz750x156.png"
```
...
```{r logo, echo=FALSE,message=FALSE, warning=FALSE,fig.align='left', out.width="400px",out.height="200px"}
...
spltcomp$Result_Text.split<-ifelse(substr(spltcomp$Result_Text.split,start=1,stop=1)!="<" & (as.numeric(spltcomp$Result_Text.split)<=spltcomp$MRLValue.split) ,paste0(spltcomp$Result_Text.split," J"),spltcomp$Result_Text.split)
...
spltrp<-subset(spltcomp,select=c(MLocID,SampleStartDate,SampleStartTime.deq,Char_Name,Char_Speciation.deq,Result_Text.deq,Result_Text.split,
                                 Result_Unit.deq,Method_Code.deq,Method_Code.split,MRLValue.deq,MRLValue.split,splitRPD,splitDiff,issue,param_grp))%>% #changed to add param_grp
  mutate(Char_Name=paste(Char_Name,' ',ifelse(is.na(Char_Speciation.deq),' ',Char_Speciation.deq))) %>%
  select(-Char_Speciation.deq)
